...
function csOpenDocModal(item) {
  ...
  overlay.innerHTML =
    ...
    +   '</div>'
    + '</div>'
    + '<div style="flex:1;display:flex;flex-direction:column;border-radius:0 0 14px 14px;overflow:hidden">'
    +   '<div style="display:flex;align-items:center;gap:8px;padding:6px 12px;background:#404040;flex-shrink:0">'
    +     '<div style="display:flex;align-items:center;gap:4px">'
    +       '<button style="width:26px;height:26px;border:none;background:rgba(255,255,255,.1);border-radius:4px;cursor:pointer;color:#d1d5db;display:flex;align-items:center;justify-content:center" title="Previous page"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><polyline points="15 18 9 12 15 6"/></svg></button>'
    +       '<div style="display:flex;align-items:center;gap:4px">'
    +         '<input value="1" style="width:28px;height:22px;border:1px solid rgba(255,255,255,.2);border-radius:3px;background:rgba(255,255,255,.1);color:#fff;font-size:11px;text-align:center;outline:none;font-family:inherit" />'
    +         '<span style="font-size:11px;color:#9ca3af">/ 3</span>'
    +       '</div>'
    +       '<button style="width:26px;height:26px;border:none;background:rgba(255,255,255,.1);border-radius:4px;cursor:pointer;color:#d1d5db;display:flex;align-items:center;justify-content:center" title="Next page"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><polyline points="9 18 15 12 9 6"/></svg></button>'
    +     '</div>'
    +     '<div style="width:1px;height:18px;background:rgba(255,255,255,.15)"></div>'
    +     '<div style="display:flex;align-items:center;gap:4px">'
    +       '<button style="width:26px;height:26px;border:none;background:rgba(255,255,255,.1);border-radius:4px;cursor:pointer;color:#d1d5db;display:flex;align-items:center;justify-content:center" title="Zoom out"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="8" y1="11" x2="14" y2="11"/></svg></button>'
    +       '<span style="font-size:11px;color:#d1d5db;min-width:36px;text-align:center">100%</span>'
    +       '<button style="width:26px;height:26px;border:none;background:rgba(255,255,255,.1);border-radius:4px;cursor:pointer;color:#d1d5db;display:flex;align-items:center;justify-content:center" title="Zoom in"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg></button>'
    +     '</div>'
    +     '<div style="flex:1;text-align:center;font-size:11px;color:#9ca3af;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + docName + '</div>'
    +   '</div>'
    +   '<div style="flex:1;background:#525659;overflow-y:auto;display:flex;flex-direction:column;align-items:center;padding:20px 0;gap:16px">'
    +     '<div style="background:#fff;width:calc(100% - 48px);max-width:560px;min-height:360px;border-radius:2px;box-shadow:0 2px 12px rgba(0,0,0,.35);padding:48px 52px;box-sizing:border-box">'
    +       '<div style="font-size:18px;font-weight:700;color:#111;margin-bottom:6px">' + (item.moments_match_analysis_name || docName).replace(/</g,'&lt;') + '</div>'
    +       '<div style="font-size:11px;color:#6b7280;margin-bottom:28px">' + (item.advertiser_name || '') + (item.campaign_name ? ' · ' + item.campaign_name : '') + '</div>'
    +       '<div style="height:1px;background:#e5e7eb;margin-bottom:24px"></div>'
    +       '<div style="display:flex;flex-direction:column;gap:8px">'
    +         ['Executive Summary','Campaign Objectives','Target Audience & Demographics','Content Strategy','Contextual Targeting Approach','Moments & Taxonomy Classification','Flight Dates & Budget Allocation','KPIs and Success Metrics'].map(function(s,i) {
                return '<div style="display:flex;gap:10px;align-items:flex-start">'
                  + '<div style="width:8px;height:8px;border-radius:50%;background:#d1d5db;flex-shrink:0;margin-top:5px"></div>'
                  + '<div><div style="font-size:12px;font-weight:600;color:#374151;margin-bottom:3px">' + s + '</div>'
                  + '<div style="height:7px;background:#f3f4f6;border-radius:3px;width:' + [92,78,85,70,88,75,82,68][i] + '%"></div>'
                  + '<div style="height:7px;background:#f3f4f6;border-radius:3px;width:' + [65,90,55,80,60,88,72,77][i] + '%;margin-top:4px"></div>'
                  + '</div></div>';
              }).join('')
    +       '</div>'
    +     '</div>'
    +     '<div style="background:#fff;width:calc(100% - 48px);max-width:560px;min-height:220px;border-radius:2px;box-shadow:0 2px 12px rgba(0,0,0,.35);padding:48px 52px;box-sizing:border-box">'
    +       '<div style="display:flex;flex-direction:column;gap:8px">'
    +         [0,1,2,3,4].map(function(_,i) {
                return '<div style="height:7px;background:#f3f4f6;border-radius:3px;width:' + [100,95,88,100,72][i] + '%"></div>';
              }).join('')
    +         '<div style="height:16px"></div>'
    +         [0,1,2,3].map(function(_,i) {
                return '<div style="height:7px;background:#f3f4f6;border-radius:3px;width:' + [100,100,90,55][i] + '%"></div>';
              }).join('')
    +       '</div>'
    +     '</div>'
    +   '</div>'
