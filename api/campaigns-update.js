...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fetch current campaign_details for merge
    const campRows = await sql`SELECT campaign_details, client_org_id, advertiser_id FROM campaigns_v2 WHERE campaign_id = ${campaignId}`;
    if (!campRows.length) return res.status(404).json({ error: 'Campaign not found' });
    const current      = campRows[0];
    const curDetails   = current.campaign_details || {};

    // ── Assign moments_match_analysis_id ─────────────────────────────────────
    if (body.moments_match_analysis_id !== undefined) {
      const aid = body.moments_match_analysis_id ? parseInt(body.moments_match_analysis_id) : null;
      const newDetails = { ...curDetails, moments_match_analysis_id: aid };
      await sql`
        UPDATE campaigns_v2
        SET campaign_details = ${JSON.stringify(newDetails)}::jsonb, updated_at = now()
        WHERE campaign_id = ${campaignId}
      `;
      // Write campaign_id back onto the moments_match row for JOIN lookups
      if (aid) {
    ...
    // ── Update name / client / advertiser ─────────────────────────────────────
    if (body.campaign_name !== undefined || body.client_name !== undefined || body.advertiser_name !== undefined) {
      let client_org_id = current.client_org_id;
      let advertiser_id = current.advertiser_id;
    ...
    // ── Update partner_ids (stored inside campaign_details JSONB) ─────────────
      ...
      if (ids.length > 0) {
        const validRows = await sql`
          ...
          WHERE connection_id = ANY(${ids})
            AND client_org_id = ${current.client_org_id}
            AND (advertiser_id IS NULL OR advertiser_id = ${current.advertiser_id})
            AND status = 'active'
        `;
        ...
        const newDetails = { ...curDetails, partner_ids: validIds };
        await sql`
          UPDATE campaigns_v2
          SET campaign_details = ${JSON.stringify(newDetails)}::jsonb, updated_at = now()
          WHERE campaign_id = ${campaignId}
        ...
        const newDetails = { ...curDetails, partner_ids: [] };
        await sql`
          UPDATE campaigns_v2
          SET campaign_details = ${JSON.stringify(newDetails)}::jsonb, updated_at = now()
          WHERE campaign_id = ${campaignId}
